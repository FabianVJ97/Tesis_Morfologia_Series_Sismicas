...
inline double step_symmetric1(double g_left,
  ...
  double c3 = g_diag + d_ij;  // diagonal (mismo costo)
...
inline void itakura_limits(int i, int n, int m,
  ...
  double upper_norm = std::min(2.0 * t, (t + 1.0) / 2.0);
    ...
    //Pasar a indices 1..m usando j_norm  (j-1)/(m-1)
  ...
  je = std::max(1, std::min(m, je));
   ...
   //asegurar al menos un indice cerca de la diagonal
  if (js > je) {
    int mid = (n > 1)
  ...
  j_end   = je;
     ...
     // [[Rcpp::export]]
double dtw_itakura_euclidiana_symmetric1_cpp(const NumericVector& A,
  ...
  C(0, 0) = 0.0;
   ...
   //DP con restriccion Itakura y patron symmetric1
  for (int i = 1; i <= n; ++i) {
    int j_start, j_end;
